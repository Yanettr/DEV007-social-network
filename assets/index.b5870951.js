...
const style = "";
const logoTitleRed = "/assets/logo-title-red.00a735f6.png";
/**
 * @license
...
if (l.document && z$1) {
  var Ea$1 = ya();
  Da = Ea$1 ? Ea$1 : parseInt(za, 10) || void 0;
} else
  Da = void 0;
var Fa = Da;
function A(a, b2) {
...
const db = Ph(app);
const updateUsername = async (name2) => {
...
const getLoggedUser = () => auth.currentUser.displayName;
const userLogout = () => signOut(auth);
const createPost = (text) => pf(_h(db, "posts"), {
  content: text,
  time: Gf(),
...
const editPost = async (newInput, docId) => {
  const docRef = gh(db, "posts", docId);
  await gf(docRef, {
    content: `${newInput}`
  });
};
const deletePost = async (docRef) => {
  const currentAuth = auth.currentUser;
  try {
...
const addLike = (id2, likes) => {
  if (likes.length === 0 || !likes.includes(auth.currentUser.email)) {
    gf(gh(db, "posts", id2), {
      likes: Qf(auth.currentUser.email)
    }).catch((error) => error);
  }
};
const removeLike = (id2) => gf(gh(db, "posts", id2), {
  likes: jf(auth.currentUser.email)
});
const redPaw = "/assets/red-paw-like.0f559e50.png";
const greyPaw = "/assets/grey-paw-like.d165b59b.png";
...
const createDeleteModal = (docId) => {
  ...
  modalMsg.innerHTML = "";
  modalMsg.innerHTML = "\xBFQuieres eliminar esta publicaci\xF3n?";
...
const createEditModal = (content, docId) => {
  ...
  cancelButton.id = "cancel-button";
  modalMsg.innerHTML = "";
  modalMsg.innerHTML = "Editar publicaci\xF3n: <br>";
...
const createPostDiv = (name2, localDate, localTime, content, docId, spanLike) => {
  ...
  userP.textContent = `${name2} public\xF3 el ${localDate} a las ${localTime} :`;
...
const spanLikeFunc = (docRef, likesArr) => {
...
const descriptionModal = () => {
  ...
  modalMsg.innerHTML = "";
  modalMsg.innerHTML = "Pets' Diaries es una aplicaci\xF3n para quienes aman a los animales y quieran compartir ese infinito cari\xF1o por ellos. Hemos creado este espacio, que funciona como una red social, para poder publicar historias graciosas, tiernas o de rescate y adopci\xF3n. Adem\xE1s de datos curiosos, tips de cuidado y consejos varios. Te invitamos a compartir una historia con nosotras. \xA1Que te diviertas!";
...
const Home = (onNavigate2) => {
  ...
  subtitle.textContent = "\xA1Bienvenid@ a la red social para los amantes de los animales!";
  ...
  loginBttn.addEventListener("click", () => onNavigate2("/login"));
  registerBttn.addEventListener("click", () => onNavigate2("/register"));
...
const logoGoogleImg = "/assets/logo_google.1811e950.png";
const Login = (onNavigate2) => {
  ...
  passwordInput.placeholder = "Contrase\xF1a";
  ...
  loginBttn.textContent = "Iniciar Sesi\xF3n";
  homeBttn.textContent = "Volver al inicio";
  heartImg.addEventListener("click", () => descModal.showModal());
  homeBttn.addEventListener("click", () => onNavigate2("/"));
...
const Register = (onNavigate2) => {
  ...
  passwordInput.type = "password";
  passwordInput.id = "myPasswordInput";
  passwordInput.placeholder = "Ingresa tu contrase\xF1a (al menos6 caracteres)";
  passwordInput.minLength = 6;
  passwordInput.required = true;
  passwordInput2.classList.add("registerInputBox");
  passwordInput2.type = "password";
  passwordInput2.id = "myPasswordInput2";
  passwordInput2.placeholder = "Repetir contrase\xF1a";
  ...
  registerBttn.addEventListener("click", (e) => {
    e.preventDefault();
    const name2 = document.getElementById("myNameInput").value;
    const email = document.getElementById("myEmailInput").value;
    ...
    if (validatePassword(password1, password2) === false) {
      alert("la contrase\xF1a ingresada no coincide");
    } else if (password1.length < 6) {
      alert("la contrase\xF1a debe tener al menos 6 caracteres");
    } else if (validateEmail(email) === false) {
      alert("la contrase\xF1a s\xED coincide pero el correo electr\xF3nico no es v\xE1lido");
    } else {
      createUser(email, password1).then(updateUsername(name2)).catch((err) => console.log("ERROR", err));
      onNavigate2("/timeline");
    }
  });
...
const Timeline = (onNavigate2) => {
  ...
  const postsRef = Rl(_h(db, "posts"), xl("time", "desc"));
  const descModal = descriptionModal();
  divUserName.classList.add("divUserName");
  divSignOut.classList.add("divSignOut");
  userName.textContent = `\xA1Bienvenid@, ${getLoggedUser()}!`;
  ...
  divUserName.addEventListener("click", () => onNavigate2("/profile"));
  userName.addEventListener("click", () => onNavigate2("/profile"));
  logoutButton.addEventListener("click", () => {
    userLogout().then(() => onNavigate2("/"));
  });
  ...
  If(postsRef, (querySnapshot) => {
    postsDiv.innerHTML = "";
    querySnapshot.forEach((post) => {
      const postContent = post.data({
        serverTimestamps: "estimate"
      });
      const name2 = post.data().displayName;
      ...
      const postDiv = createPostDiv(name2, localDate, localTime, content, docId, spanLike);
...
const Profile = (onNavigate2) => {
  ...
  const heartImg = document.createElement("img");
  const descModal = descriptionModal();
  const updateNameModal = updateUsernameModal();
  heartImg.src = `${logoTitleRed}`;
  heartImg.classList.add("heart");
  ...
  descModal.id = "about-modal";
  heartImg.addEventListener("click", () => descModal.showModal());
  subtitle.textContent = `\xA1Bienvenid@, ${getLoggedUser()}! Este es tu perfil.`;
  updateUsernameButton.textContent = "Cambiar nombre";
  updateUsernameButton.addEventListener("click", () => updateNameModal.showModal());
  homeBttn.textContent = "Volver atr\xE1s";
  homeBttn.addEventListener("click", () => onNavigate2("/timeline"));
  logoutButton.textContent = "Cerrar sesi\xF3n";
  logoutButton.addEventListener("click", () => {
    userLogout().then(() => onNavigate2("/"));
  });
